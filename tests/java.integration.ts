...
async function digest(root: string): Promise<string> {
  ...
  for (const file of await listProjectFiles(root)) hash.update(file).update(await readFile(path.join(root, file)));
...
async function freePort(): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    ...
    probe.once('error', () => reject(new Error('Port 4329 is occupied; the existing process was not touched.')));
    probe.listen({ host: '127.0.0.1', port: 4329, exclusive: true }, () => probe.close(error => error ? reject(error) : resolve()));
  });
}
...
async function main(): Promise<void> {
  ...
  const summary: Record<string, unknown> = {
    status: 'running', startedAt: new Date().toISOString(), requestedVersion: version,
    testedVersion: version, pinnedVersionVerified: false, versionSource: 'examples/java/pom.xml',
    versionOverride: false,
    port: 4329, workspace, runDir, sourceDigestsBefore: before, commands: [],
  };
  ...
  async function mvn(label: string, args: string[]) {
    ...
    const command = ['-B', '-ntp', `-Dmaven.repo.local=${path.join(work, 'java-maven-cache')}`, ...args];
  ...
  async function start(broken: boolean) {
    await freePort();
    const env: NodeJS.ProcessEnv = { ...process.env, PORT: '4329', JOURNEYPROOF_BROKEN_DISCOUNT: broken ? '1' : '0' };
    delete env.ELECTRON_RUN_AS_NODE;
    server = spawn(process.execPath, ['server.mjs'], { cwd: cartCopy, env, shell: false, stdio: ['ignore', 'pipe', 'pipe'] });
    const child = server;
    await new Promise<void>((resolve, reject) => {
      let output = '';
      const timer = setTimeout(() => finish(new Error(`Cart readiness timeout: ${output}`)), 10_000);
      const exit = () => finish(new Error(`Cart failed to start: ${output}`));
      const error = (e: Error) => finish(e);
      const consume = (chunk: Buffer) => {
        output += chunk.toString();
        if (output.includes('JourneyProof cart ready at http://127.0.0.1:4329')) finish();
      };
      function finish(e?: Error) {
        clearTimeout(timer); child.off('exit', exit); child.off('error', error);
  ...
  try {
    ...
    const install = await mvn('browser-install', ['exec:java', '-Dexec.mainClass=com.microsoft.playwright.CLI',
    ...
    if (install.code !== 0 || install.timedOut || install.cancelled) {
      ...
      throw new Error(`Java dependency/browser setup failed for Playwright ${version}. See browser-install.log.`);
    }
    ...
    await start(false);
    recorder = new BrowserRecorder({ artifactDir: path.join(runDir, 'recording'), onEvent: () => {} });
    ...
    const recording = await recorder.stop(); recorder = undefined;
    const scenario: Scenario = { schemaVersion: 1, id: randomUUID(), name: 'Java cart coupon', startUrl: url,
      createdAt: new Date().toISOString(), ...recording,
      assertions: [{ id: 'discount-total', description: 'SAVE10 reduces the $100 cart total to $90.00.',
    ...
    for (const id of ['add-notebook', 'add-bag', 'coupon', 'apply-coupon']) {
      assert(scenario.events.some(event => event.locators.some(locator => locator.strategy === 'testId' && locator.value === id)), `Missing recorded ${id}`);
    }
    ...
    const healthy = await mvn('healthy', ['test', `-Dtest=journeyproof.${className}`]);
    assert.equal(healthy.code, 0, 'Generated Java coupon test failed on the healthy cart');
    assert.match(healthy.output, /Tests run: 1, Failures: 0, Errors: 0, Skipped: 0/);
    await writeFile(path.join(runDir, 'healthy-surefire.xml'), await readFile(path.join(workspace, 'target/surefire-reports', `TEST-journeyproof.${className}.xml`)));
    await stop(server); server = undefined;
    await start(true);
    const broken = await mvn('broken', ['test', `-Dtest=journeyproof.${className}`]);
    assert.equal(broken.code, 1, 'Mutation must complete with a test failure');
    assert.equal(broken.timedOut, false);
    const report = await readFile(path.join(workspace, 'target/surefire-reports', `TEST-journeyproof.${className}.xml`), 'utf8');
    await writeFile(path.join(runDir, 'broken-surefire.xml'), report);
    assert.match(report, /tests="1"/); assert.match(report, /failures="1"/);
    assert.match(report, /errors="0"/); assert.match(report, /skipped="0"/);
    assert.match(report, /\$90\.00/); assert.match(report, /\$95\.00/);
    assert.match(report, /AssertionFailedError|AssertionError/);
    assert.equal(await readFile(path.join(workspace, file.path), 'utf8'), file.content, 'Generated code changed between runs');
    ...
    if (!summary.originalUnchanged) { summary.status = 'failed'; failure = new Error('Original source changed during integration.'); }
    ...
    console.log(JSON.stringify({ status: summary.status, testedVersion: version, pinnedVersionVerified: summary.pinnedVersionVerified,
      originalUnchanged: summary.originalUnchanged, evidence: path.join(work, 'java-verification.json'), error: summary.error }, null, 2));
  }
