...
function running(pid: number): boolean {
  try { process.kill(pid, 0); } catch (error: any) { if (error.code === 'ESRCH') return false; throw error; }
...
for (const mode of ['cancel', 'timeout'] as const) {
  test(`${mode} waits for the owned detached group after its leader exits`, { skip: process.platform === 'win32', timeout: 12_000 }, async () => {
    ...
    try {
      const result = await runProcess(process.execPath, ['-e', leaderCode], {
        cwd: tmpdir(), signal: controller.signal, timeoutMs: mode === 'timeout' ? 1000 : 7000,
        onOutput: text => {
          output += text;
      ...
      assert.ok(descendant && Number.isSafeInteger(descendant), 'descendant announced readiness before termination');
      ...
      if (descendant && running(descendant)) { try { process.kill(descendant, 'SIGKILL'); } catch {} }
      if (sentinel.exitCode === null && sentinel.signalCode === null) {
        const exited = once(sentinel, 'exit'); sentinel.kill('SIGKILL'); await exited;
      }
    }
  });
}
