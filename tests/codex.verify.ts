...
const healthy = await verifyWorkspace(data.workspace, command, 2, {
  prepareDemo: true,
  env: { PORT: '4331' },
...
assert.ok(
  healthy.runs.every((run) => run.output.includes('SAVE10')),
  'Generated test discovered',
);
const mutant = await verifyWorkspace(data.workspace, command, 1, {
  env: { PORT: '4331', JOURNEYPROOF_BROKEN_DISCOUNT: '1' },
  onProgress: console.log,
});
...
console.log(
  JSON.stringify(
    { healthy: healthy.status, repeats: healthy.runs.length, mutant: mutant.status },
    null,
    2,
  ),
);
