...
async function requireFreePort(): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    ...
    probe.once('error', () =>
      reject(
        new Error(
          'Port 4318 is occupied. Stop the other run before this integration; its server was not touched.',
        ),
      ),
    );
    probe.listen({ host: '127.0.0.1', port: 4318, exclusive: true }, () =>
...
async function waitForServer(child: ChildProcess): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    ...
    const timer = setTimeout(
      () => finish(new Error(`Recording server did not become ready: ${output}`)),
      10_000,
    );
    ...
    const consume = (chunk: Buffer) => {
      ...
      if (output.includes('Testloom cart ready at http://127.0.0.1:4318')) finish();
    };
  ...
  try {
    ...
    assert(
      project.commands.some(
        (candidate) => candidate.executable === 'npm' && candidate.args.join(' ') === 'test',
      ),
    );
    ...
    const serverEnv: NodeJS.ProcessEnv = {
      ...process.env,
      PORT: '4318',
      JOURNEYPROOF_BROKEN_DISCOUNT: '0',
    };
    ...
    recorder = new BrowserRecorder({
      artifactDir: path.join(temp, 'screenshots'),
    ...
    const order = recording.events.map((event) => ({
      action: event.action,
      target: event.locators[0]?.value ?? '',
      value: event.value ?? '',
    }));
    ...
    assert.deepEqual(
      recording.events.map((event) => event.sequence),
      [1, 2, 3, 4, 5],
    );
    assert(
    ...
    assert.equal(
      recording.network.filter(
        (entry) =>
          entry.method === 'POST' && entry.url === `${startUrl}api/cart` && entry.status === 200,
      ).length,
      3,
    );
    const scenario: Scenario = {
      schemaVersion: 1,
      id: 'cart-integration-save10-0001',
      name: 'Recorded cart SAVE10 totals ninety dollars',
      startUrl,
      createdAt: startedAt,
      ...recording,
      assertions: [
        {
          id: 'cart-total-90',
          description: 'Both products with SAVE10 total exactly $90.00',
          kind: 'text',
    ...
    await requireFreePort();
    const broken: Verification = await verifyWorkspace(workspace, command, 1, {
      ...options,
      env: { ...env, JOURNEYPROOF_BROKEN_DISCOUNT: '1' },
    });
    ...
    assert.equal(
      await readFile(path.join(workspace, generated.files[0].path), 'utf8'),
      code,
      'Mutation verification changed the assertion',
    );
    ...
    if (before !== after) {
      failure ??= new Error('Original cart source digest changed');
      summary.status = 'failed';
    }
    ...
    console.log(
      `COMPLETE: ${summary.status}; temporary workspace removed. Summary: ${summaryPath}`,
    );
  }
