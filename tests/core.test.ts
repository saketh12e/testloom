...
const scenario: Scenario = {
  ...
  assertions: [
    {
      id: 'a1',
      description: 'The discount total is $90',
      kind: 'text',
...
test('snapshot excludes credentials, symlinks, dependency caches and preserves source bytes', async (t) => {
  const root = await mkdtemp(path.join(tmpdir(), 'journeyproof-core-'));
...
test('generation path boundary rejects traversal, source edits, case collisions and unexpected extensions', () => {
  for (const bad of [
    '../x.ts',
    'tests/journeyproof/../../src/app.ts',
    '/tests/journeyproof/a.ts',
    'tests/journeyproof/a.sh',
    'tests/journeyproof/a\\b.ts',
    'src/main.ts',
  ])
    assert.throws(() => validateGeneratedFiles([{ path: bad, content: 'x' }], project.outputDir));
  assert.throws(() =>
    validateGeneratedFiles(
      [
        { path: 'tests/journeyproof/A.ts', content: 'x' },
        { path: 'tests/journeyproof/a.ts', content: 'x' },
      ],
      project.outputDir,
    ),
...
test('file application rejects symlink escape and overwrite', async (t) => {
  const root = await mkdtemp(path.join(tmpdir(), 'journeyproof-path-'));
  t.after(() => rm(root, { recursive: true, force: true }));
  await mkdir(path.join(root, '.journeyproof'));
  await mkdir(path.join(root, 'tests'));
  await symlink(tmpdir(), path.join(root, 'tests/journeyproof'));
  ...
  await rm(path.join(root, 'tests/journeyproof'));
  await mkdir(path.join(root, 'tests/journeyproof'));
  await writeFile(path.join(root, 'tests/journeyproof/a.ts'), 'original');
  await assert.rejects(
    writeGeneratedFiles(
      root,
      [{ path: 'tests/journeyproof/a.ts', content: 'new' }],
      project.outputDir,
    ),
    /overwrite/,
  );
...
test('portable compiler preserves explicit expected values and safe quoting in TS and Java', () => {
  ...
  s.name = 'A "quoted" journey';
  const ts = portableGenerate(project, s).files[0].content;
  assert.match(ts, /toHaveText\("\$90\.00"\)/);
  assert.match(ts, /Requirement a1/);
  assert.ok(!ts.includes('waitForTimeout'));
  const java = portableGenerate(
    { ...project, framework: 'playwright-java', outputDir: 'src/test/java/journeyproof' },
    s,
  ).files[0];
  assert.match(java.content, /hasText\("\$90\.00"\)/);
  assert.match(java.path, /Test\.java$/);
});
test('navigation observations become assertions rather than bypassing a broken click', () => {
  const s = structuredClone(scenario);
  s.events.push({ ...s.events[0], id: 'e3', sequence: 3, url: 'https://example.com/cart' });
...
test('missing requirements, unsupported steps, redacted credentials and custom rules fail closed', () => {
  ...
  assert.throws(
    () => portableGenerate(project, { ...scenario, warnings: ['file upload'] }),
    /unsupported/,
  );
  const secret = structuredClone(scenario);
  secret.events[1].redacted = true;
  assert.throws(() => portableGenerate(project, secret), /redacted/);
  const custom = structuredClone(scenario);
  custom.assertions[0].kind = 'custom';
...
test('Codex prompt excludes screenshot files and labels recorded data untrusted', () => {
  const s = structuredClone(scenario);
  s.events[0].screenshot = '/private/customer.png';
  const prompt = generationPrompt(project, s);
  assert.ok(!prompt.includes('/private/customer.png'));
...
test('execution evidence cannot be exported against changed generated bytes', async (t) => {
  const root = await mkdtemp(path.join(tmpdir(), 'journeyproof-bytes-'));
  t.after(() => rm(root, { recursive: true, force: true }));
  await mkdir(path.join(root, '.journeyproof'));
  ...
  const hashes = await verifyGeneratedBytes(root, files);
  assert.equal(hashes[files[0].path].length, 64);
...
test('test discovery rejects skipped, retry-only, missing and expected-failure reports', () => {
  ...
  assert.equal(
    inspectPlaywrightReport(report, ['tests/journeyproof/a.spec.ts'], '/repo').length,
    1,
  );
  for (const kind of ['skipped', 'failed']) {
    const r = structuredClone(report);
    r.suites[0].specs[0].tests[0].results[0].status = kind;
    assert.throws(() => inspectPlaywrightReport(r, ['tests/journeyproof/a.spec.ts'], '/repo'));
  }
  const retried = structuredClone(report);
  retried.suites[0].specs[0].tests[0].results.unshift({ status: 'failed' });
  assert.throws(() => inspectPlaywrightReport(retried, ['tests/journeyproof/a.spec.ts'], '/repo'));
  assert.throws(() =>
    inspectPlaywrightReport(report, ['tests/journeyproof/missing.spec.ts'], '/repo'),
  );
});
test('process execution preserves literal arguments, cancellation and timeouts', async () => {
  ...
  const result = await runProcess(
    process.execPath,
    ['-e', 'console.log(process.argv[1])', literal],
    { cwd: tmpdir() },
  );
  assert.equal(result.code, 0);
  assert.equal(result.output.trim(), literal);
  const timed = await runProcess(process.execPath, ['-e', 'setInterval(()=>{},1000)'], {
    cwd: tmpdir(),
    timeoutMs: 50,
  });
  ...
  const cancelled = await runProcess(process.execPath, ['-e', 'setInterval(()=>{},1000)'], {
    cwd: tmpdir(),
...
test('verification reports completed failures separately from environment errors and never retries failed checks', async (t) => {
  const root = await mkdtemp(path.join(tmpdir(), 'journeyproof-verify-'));
  t.after(() => rm(root, { recursive: true, force: true }));
  await mkdir(path.join(root, '.journeyproof'));
  const pass = await verifyWorkspace(
    root,
    {
      ...
      args: ['-e', 'console.log("one executed assertion")'],
      label: 'test',
    },
    2,
  );
  assert.equal(pass.status, 'passed');
  assert.equal(pass.runs.length, 2);
  const fail = await verifyWorkspace(
    root,
    { executable: process.execPath, args: ['-e', 'process.exit(1)'], label: 'test' },
    3,
  );
  assert.equal(fail.status, 'failed');
  assert.equal(fail.runs.length, 1);
  const missing = await verifyWorkspace(
    root,
    { executable: 'journeyproof-nonexistent-test-runner', args: [], label: 'missing' },
    1,
  );
  assert.equal(missing.status, 'environment-error');
});

test('empty-text outcomes remain exact assertions and imported IDs do not alias Java classes', () => {
  const s = structuredClone(scenario);
  s.assertions[0].expected = '';
  const empty = portableGenerate(project, s);
  assert.match(empty.files[0].content, /toHaveText\(""\)/);
  const java = {
    ...project,
    framework: 'playwright-java' as const,
    outputDir: 'src/test/java/testloom',
  };
  const first = portableGenerate(java, { ...s, id: 'abcdefgh-one' }).files[0];
  const second = portableGenerate(java, { ...s, id: 'abcdefgh-two' }).files[0];
  assert.notEqual(first.path, second.path);
  assert.notEqual(
    /public class (\w+)/.exec(first.content)?.[1],
    /public class (\w+)/.exec(second.content)?.[1],
  );
});
