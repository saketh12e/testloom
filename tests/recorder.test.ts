...
before(async () => {
  server = createServer((request, response) => {
    ...
    if (request.url?.startsWith('/api')) {
      ...
      response.end(JSON.stringify({ secret: 'response-body-secret' }));
      return;
    }
...
async function fixture(screenshots = false) {
  ...
  await recorder.start(`${origin}/?token=start-secret#private`, {
    headless: true,
...
test(
  'real browser coalesces typing and records click, select, check and semantic press in order',
  { timeout: 30_000 },
  async () => {
    ...
    try {
      ...
      assert.deepEqual(
        ...
        ['navigate', 'fill', 'click', 'check', 'uncheck', 'select', 'fill', 'press'],
      );
      assert.equal(result.events[1].value, 'Journey');
      assert.deepEqual(
        result.events[1].locators.map((locator) => locator.strategy),
        ['testId', 'role', 'label', 'css'],
      );
      ...
      assert(
        result.events.every(
          (event, index) => !index || event.timestamp >= result.events[index - 1].timestamp,
        ),
      );
    } finally {
      await f.cleanup();
...
test(
  'sensitive values are redacted before callback delivery, including the final unblurred edit',
  { timeout: 30_000 },
  async () => {
    const f = await fixture();
    try {
...
test(
  'navigation and request ledger preserve order and strip query, fragment and authentication',
  { timeout: 30_000 },
  async () => {
    const f = await fixture();
    try {
      ...
      await page.evaluate(async () => {
        await fetch('/api?token=request-secret', {
          method: 'POST',
      ...
      await page.goto(
        origin.replace('http://', 'http://user:auth-secret@') +
          '/authenticated?token=secret#fragment',
      );
      const result = await f.recorder.stop();
      assert.deepEqual(
        result.events.filter((event) => event.action === 'navigate').map((event) => event.url),
        [`${origin}/`, `${origin}/next`, `${origin}/spa`, `${origin}/authenticated`],
      );
      assert.deepEqual(
        result.events.slice(0, 4).map((event) => event.action),
        ['navigate', 'fill', 'click', 'navigate'],
      );
      const api = result.network.find((entry) => entry.method === 'POST');
      assert.deepEqual(api, { method: 'POST', url: `${origin}/api`, status: 201 });
      assert(
        result.network.every(
          (entry) => Object.keys(entry).sort().join(',') === 'method,status,url',
        ),
      );
      ...
      for (const secret of [
        ...
        assert(!JSON.stringify(result).includes(secret), `leaked ${secret}`);
    } finally {
      await f.cleanup();
    }
  },
...
test(
  'keyboard button activation is recorded once and later sensitive edits remain distinct',
  { timeout: 30_000 },
  async () => {
    const f = await fixture();
    try {
      ...
      assert.deepEqual(
        result.events.map((event) => event.action),
        ['navigate', 'fill', 'press', 'fill', 'press'],
      );
      assert.deepEqual(
        result.events.filter((event) => event.action === 'press').map((event) => event.value),
        ['Enter', 'Space'],
      );
...
test(
  'unsupported uploads, canvas, drag and frames emit explicit warnings',
  { timeout: 30_000 },
  async () => {
    const f = await fixture();
    try {
      ...
      await chooser.setFiles({
        name: 'private.txt',
        mimeType: 'text/plain',
        buffer: Buffer.from('file-body-secret'),
      });
      await page.locator('#canvas').click();
      await page.locator('#drag').dragTo(page.locator('#drop'));
      await page.evaluate(() => {
        const frame = window.document.createElement('iframe');
        frame.srcdoc = '<input id="private">';
        window.document.body.append(frame);
      });
      await page.frameLocator('iframe').locator('#private').fill('frame-secret');
      const result = await f.recorder.stop();
      for (const category of ['File uploads', 'Canvas', 'Drag and drop', 'Frames'])
        assert(
          result.warnings.some((warning) => warning.includes(category)),
          `missing ${category}`,
        );
      assert(
        !result.events.some(
          (event) =>
            event.action === 'click' &&
            event.locators.some((locator) => locator.value === '#canvas'),
        ),
      );
      for (const secret of ['private.txt', 'file-body-secret', 'frame-secret'])
...
test(
  'stop flushes pending input, closes the browser, is idempotent and permits a fresh session',
  { timeout: 30_000 },
  async () => {
    const f = await fixture();
    try {
      ...
      assert.equal(
        await f.recorder.page.evaluate(() => localStorage.getItem('privateSession')),
        null,
      );
...
test(
  'recording cancellation interrupts a pending navigation and closes its browser',
  { timeout: 15_000 },
  async () => {
...
test(
  'immediate cancellation during browser launch leaves no active session or late callbacks',
  { timeout: 15_000 },
  async () => {
    const artifactDir = await mkdtemp(join(tmpdir(), 'journeyproof-cancel-'));
...
test(
  'screenshots require opt-in, warn about visible data and mask inputs and marked content',
  { timeout: 30_000 },
  async () => {
    ...
    try {
      ...
      page.screenshot = async (options) => {
        masks.push(
          await options!.mask![0].evaluateAll((elements) =>
            elements.map((el) => el.id || el.getAttribute('data-sensitive') || el.localName),
          ),
        );
      ...
      for (const event of screenshots)
        assert.equal((await readFile(event.screenshot!)).subarray(1, 4).toString(), 'PNG');
...
test(
  'page-discovered bridge rejects forged events without the private nonce',
  { timeout: 15_000 },
  async () => {
    const f = await fixture();
    try {
      await f.recorder.page.evaluate(async () => {
        ...
        const fake = {
          action: 'fill',
          url: location.href,
          label: 'Forged password',
          locators: [{ strategy: 'testId', value: 'password' }],
          value: 'forged-plaintext-secret',
          redacted: false,
        };
...
test(
  'replacing the global binding cannot intercept real events or the nonce',
  { timeout: 15_000 },
  async () => {
    const f = await fixture();
    try {
      ...
      await page.getByTestId('save').click();
      assert.deepEqual(
        await page.evaluate(() => (window as unknown as { stolen: unknown[] }).stolen),
        [],
      );
      const result = await f.recorder.stop();
      assert.deepEqual(
...
test(
  'hostile stop replacement is blocked and a poisoned flush cannot hang cancellation',
  { timeout: 15_000 },
  async () => {
    const f = await fixture();
    try {
      ...
      const defence = await page.evaluate(async () => {
        const host = window as unknown as Record<string, unknown>;
        const control = Object.getOwnPropertyNames(window).find((key) =>
          key.startsWith('__journeyproof_stop_'),
        )!;
      ...
      await page.evaluate(() => {
        Promise.all = (() => new Promise(() => {})) as typeof Promise.all;
      });
...
test(
  'native packet validation rejects oversized values and malformed locators',
  { timeout: 15_000 },
  async () => {
    const f = await fixture();
    try {
      ...
      await f.recorder.page.evaluate(async (token) => {
        ...
        const packet = {
          ...
          url: location.origin,
          label: 'bad',
          locators: [{ strategy: 'execute', value: 'bad' }],
        };
        await host[binding](token, packet);
        await host[binding](token, { ...packet, action: 'execute', locators: [] });
        await host[binding](token, {
          ...packet,
          locators: Array(9).fill({ strategy: 'css', value: 'body' }),
        });
      }, nonce);
      const result = await f.recorder.stop();
      assert.deepEqual(
...
test(
  'native event reservations and network ledger stop at their documented caps',
  { timeout: 30_000 },
  async () => {
    const f = await fixture();
    try {
      const page = f.recorder.page;
      await page.evaluate(() => {
        for (let index = 0; index < 510; index++) history.pushState({}, '', `/step-${index}`);
      });
      await page.evaluate(async () => {
        for (let batch = 0; batch < 21; batch++)
          await Promise.all(
            Array.from({ length: 100 }, (_, index) => fetch(`/api?batch=${batch}&index=${index}`)),
          );
      });
      const result = await f.recorder.stop();
