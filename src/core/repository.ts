...
export async function listProjectFiles(root: string, maxFiles = 8000): Promise<string[]> {
  ...
  async function walk(dir: string, depth: number): Promise<void> {
    if (depth > 16)
      throw new Error(
        'Project exceeds the supported directory depth. Choose a smaller module folder.',
      );
    for (const entry of (await readdir(dir, { withFileTypes: true })).sort((a, b) =>
      ...
      if (entry.isDirectory()) await walk(full, depth + 1);
        ...
        if (found.length > maxFiles)
          throw new Error(
            'This folder contains more than 8,000 source files. Choose the test module folder.',
          );
      }
    }
  }
...
export async function inspectProject(input: string): Promise<Project> {
  ...
  const read = async (f: string) => {
    try {
      return (await readFile(path.join(root, f), 'utf8')).slice(0, 40_000);
    } catch {
      return '';
    }
  };
  ...
  try {
    ...
    throw new Error('The project package.json is not valid JSON.');
  }
  const javaBuild =
  ...
  const testFiles = files
    .filter((f) => /(?:\.spec\.[jt]sx?$|\.test\.[jt]sx?$|Test\.java$|src\/test\/.*\.java$)/.test(f))
    .slice(0, 5);
  const helperFiles = files
    .filter(
      (f) => /(?:pages?|fixtures?|support|helpers?)\//i.test(f) && /\.(?:java|[jt]s)$/.test(f),
    )
    .slice(0, 4);
  const configFiles = [
    'package.json',
  ...
  const framework = java
    ? /playwright/i.test(clues)
      ...
      : /selenium/i.test(clues)
        ? 'selenium-java'
        : 'unknown'
    : /playwright/i.test(clues) || files.some((f) => /playwright\.config/.test(f))
      ? 'playwright-ts'
      : 'unknown';
  ...
  const commands = java
    ? [
        {
          executable: files.includes('pom.xml')
            ? files.includes('mvnw')
              ? './mvnw'
              : 'mvn'
...
export async function snapshotProject(
  ...
  for (const file of files) {
    ...
    if (!info.isFile() || info.isSymbolicLink())
      throw new Error('Project changed during snapshot. Please retry.');
    total += info.size;
    if (info.size > 20_000_000 || total > 250_000_000)
      throw new Error('Project exceeds the 250 MB source snapshot limit. Select a smaller module.');
  ...
  await writeFile(
    path.join(target, '.journeyproof/provenance.json'),
    JSON.stringify(
      {
        source: root,
        digest,
