...
## Recording runtime and release boundary (0.4)

A packaged Mac app owns a full pinned Chromium distribution in `Contents/Resources/recording-browser`. Its manifest binds the Playwright version, Chromium version and CPU architecture to an executable inside the bundle. Recording selects that path explicitly, independent of the user's browser cache. Source development uses the normal Playwright cache.

Startup performs platform and manifest checks, launches a fresh owned browser, and probes a blank page before any requested URL is opened. A candidate has a 15-second launch limit and an 8-second readiness limit; candidate admission stops after 45 seconds. Cleanup and a final admitted readiness check can extend that budget. Cancellation closes owned sessions and suppresses further candidates; a launch already in progress must settle within its timeout. Missing or failed candidates can recover through standard and per-user Chrome/Edge installations. Explicit policy denial stops fallback. Navigation errors are handled outside the candidate loop, so recovery cannot replay a customer's requested page.

The recorder creates one structured report per start. A bounded, allowlisted parser extracts only failure categories and available exit codes/signals; browser logs never enter the report. The service persists the latest report with private permissions and reconstructs validated fields before native export. Progress uses the existing renderer event stream, and the recording dialog exposes cancellation and report export.

`package-mac.mjs` prepares the complete matching browser in isolated staging, builds the app, and packages one architecture. Preparation never overwrites an unverified existing directory or modifies a global browser cache. The release hooks validate full resources, Mach-O architecture, pinned dependency versions and the app signature both before archiving and after extraction. The packaged integration gate then records a synthetic page from the actual ZIP with an empty browser cache and no Node on the app's PATH. Native ARM and Intel CI jobs repeat that gate.

Compatibility policy follows the pinned [Playwright system requirements](https://playwright.dev/docs/intro#system-requirements), with [enterprise browser restrictions](https://playwright.dev/docs/browsers#google-chrome--microsoft-edge) respected. Native CI runner labels are documented by [GitHub](https://docs.github.com/en/actions/reference/runners/github-hosted-runners). Dependency or runtime upgrades must retain these gates.
