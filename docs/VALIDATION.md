...
The first native Intel run identified a large-response parser that repeatedly rescanned the growing record until a transport timeout fired. The parser now scans only newly decoded text and maintains incremental byte counts. The original 8 MB record limit and the 10-second rejection test remain unchanged; an additional fragmented multi-byte record checks exact byte-boundary behavior.
